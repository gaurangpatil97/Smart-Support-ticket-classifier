...
X = st_model.encode(df['text'].tolist(), show_progress_bar=False)
preds = clf.predict(X)
df['predicted'] = preds
...
print("\n===== CLASS REPORT =====")
print(classification_report(df['true_literal'] if 'true_literal' in df else df['true_label'], df['predicted']))